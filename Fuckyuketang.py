...
def get_course_list():
    params = {'identity': '2'}
    response = requests.get('https://www.yuketang.cn/v2/api/web/courses/list', params=params,
...
def get_video_watch_progress(cid, user_id, classroomid, video_id):
    url = f"https://www.yuketang.cn/video-log/get_video_watch_progress/?cid={cid}&user_id={user_id}&classroom_id={classroomid}&video_type=video&vtype=rate&video_id={video_id}&snapshot=1&term=latest&uv_id=3714"
...
def lookok(video_id, video_name, cid, user_id, classroomid, skuid):
    ...
    get_url = f"https://www.yuketang.cn/video-log/get_video_watch_progress/?cid={cid}&user_id={user_id}&classroom_id={classroomid}&video_type=video&vtype=rate&video_id={video_id}&snapshot=1&term=latest&uv_id=3714"
...
if __name__ == "__main__":
    ...
    if select:
        skuid = requests.get(url='https://www.yuketang.cn/v2/api/web/classrooms/%s?role=5' % cid,
        ...
        response = requests.post('https://www.yuketang.cn/mooc-api/v1/lms/learn/course/pub_new_pro',
                                 headers=headers, data=json.dumps(data)).json()
        for i in response['data']:
            if response['data'][i]['total_done'] == 1:
                print("%s你偷偷看了网课？，跳过...." % Unitmap[i])
                continue
            url = 'https://www.yuketang.cn/c27/online_courseware/xty/kls/pub_news/%s/' % i
            ret = requests.get(url, headers=headers).json()
            for j in ret['data']['content_info'][0]['section_list']:
                VideoId = j['leaf_list'][0]['id']
                get_url = 'https://www.yuketang.cn/mooc-api/v1/lms/learn/leaf_info/%s/%s/' % (cid, VideoId)
                getccid = requests.get(url=get_url, headers=headers).json()
                skuid = getccid['data']['sku_id']
                user_id = getccid['data']['user_id']
                course_id = getccid['data']['course_id']
